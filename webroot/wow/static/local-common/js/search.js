...
var TypeAhead = Class.extend({
	...
	trigger: function(e) {
		...
		if (code == KeyCode.arrowDown || code == KeyCode.arrowUp) {
            ...
            text = cache[this.position].title;
			ghostText = '';

		// Pre-select first item
		} else if (code == KeyCode.tab) {
            text = cache[0].title;
			ghostText = cache[0].title;
	...
	_populate: function() {
		...
		if (this.ghost && counter) {
			var text = this.cache[0].title;
