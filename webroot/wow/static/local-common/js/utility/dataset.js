 ...
 *		</div>
 *
 *		var dataSet = new DataSet('#data-set');
 */
...
var DataSet = Class.extend({
	...
	init: function(query, config) {
		...
		var container = this.node.find(this.config.elementContainer);
		...
		if (container.length) {
			...
			} else if (container.find('ul').length) {
				this.isList = true;
	...
	cache: function(recache) {
		...
		var row = null,
			rows = this.container.find(this.config.elementRow).detach(),
			subrows = [],
			dataImplicit = [],
			dataExplicit = [],
			source = [];
	...
	process: function(filtered) {
			...
			for (i = 0; i <= length; i++) {
				...
				append = true;
					...
					if (append) {
						...
						if (config.altRows) {
						row.removeClass('row1 row2').addClass('row'+ ((i % 2) ? 1 : 2));
						}
		...
		if (this.isTable) {
			this.container.find('tbody').html(fragment);

		} else if (this.isList) {
			this.container.find('ul').html(fragment);
	...
	setup: function() {
		...
		if (config.sorting && this.isTable) {
			this.headers = this.container.find('thead th');
			this.links = this.headers.find('a.sort-link');

			if (this.links.length) {
				this.links.unbind().bind('click', { instance: this }, function(e) {
					e.stopPropagation();
					...
					instance.sort(method, null, instance.headers.index(node.parent('th')));
	...
	_buildLink: function(text, page) {
		...
		.bind('click', { instance: this }, function(e) {
			e.preventDefault();
			e.data.instance.paginate(page);
	...
	_processFilters: function(row, cache) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (!map[key] || i != map[key] || rule.match === "")
				continue;
			...
			// Class
			} else if (rule.filter === 'class' && !row.hasClass(rule.match)) {
				pass = false;
			}
