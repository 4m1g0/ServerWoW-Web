var GalleryViewer = {
     ...
     * Constants
     */
    THUMBNAIL_HOLDER_HEIGHT: 630, //total height of thumbnail container
    THUMBNAIL_HEIGHT:  90, //total height including borders, margin, and padding
    ...
    loadItem: function(nodeId) {
        ...
        if (oldIndex != newIndex) {
            ...
            if (GalleryViewer.galleryType == "images") {
                ...
                tempImage.src = itemPaths[newIndex] + ".jpg";
    ...
    imageLoader: function(loadingImage) {
        if (loadingImage.complete) {
            ...
            setTimeout(function() { GalleryViewer.imageLoader(loadingImage) }, 100);
        }
    },
    setVideo: function(itemIndex) {
        ...
        swfobject.embedSWF(Flash.videoPlayer + noCache, "flash-video", currentVideoData.w, currentVideoData.h,
                Flash.requiredVersion, Flash.expressInstall, newFlashVars, Flash.defaultVideoParams);
    },
    /*
     ...
     * @param int[] itemIndices
     */
    preloadMetaData: function(itemIndices) {
        if (itemIndices.length > 0) {
            var nodeId = indices[itemIndices.pop()];

            if (!GalleryViewer.filmStripCache[nodeId]) {
                $.ajax({
                    type: "GET",
                    url: Core.baseUrl + "/" + GalleryViewer.AJAX_URL,
                    data: ( { id: nodeId, dataKey: GalleryViewer.dataKey, preload: "preload" } ),
                    dataType: "html",
                    success: function(msg) {
                        GalleryViewer.filmStripCache[nodeId] = msg;
                        GalleryViewer.preloadMetaData(itemIndices);
    ...
    loadPage: function(page) {
        ...
        if (GalleryViewer.thumbnailPageCache[page]) {
            ...
            $.ajax({
                ...
                data: ( { page: page, dataKey: dataKey, keywords: keywordParameter } ),
                dataType: "html",
                success: function(msg) {
    ...
    nudgeScrollContent: function(margin, oldIndex, newIndex, imageSize) {
        ...
        if (currentOffset > topLimit) {
            GalleryViewer.filmStripThumbnails.css("top", (topLimit * -1) + "px");
            GalleryViewer.scrollThumb.css("top", (topLimit / scrollBarRatio) + "px");
        }
        ...
        if (newIndex > 6) {
            ...
            if (currentOffset < bottomLimit) {
                GalleryViewer.filmStripThumbnails.css("top", (bottomLimit * -1) + "px");
                GalleryViewer.scrollThumb.css("top", (bottomLimit / scrollBarRatio) + "px");
            }
        }
    },
    ...
    loadFilmStripThumbnails: function(offsetTop) {
        ...
        GalleryViewer.slideTimeout = setTimeout(function() {
            ...
            for (var x = minThumbnailIndex; x <= maxThumbnailIndex; x++) {
                if (GalleryViewer.thumbnailLinkNodes[x].hasClass("film-strip-thumbnail-loader")) {
    ...
    thumbSlide: function() {
      clearTimeout(GalleryViewer.slideTimeout);
      GalleryViewer.slideTimeout = setTimeout(function() { GalleryViewer.loadFilmStripThumbnails() } , 100);
    },
    setImageBackground: function(src, target) {
        target.css("background-image", "url(' " + src + "')").removeClass("film-strip-thumbnail-loader");
    ...
    if (viewType == "film-strip") {
        ...
        if (indices.length > 7) {
            ...
            $(document).ready(function(){
                oScrollbar.tinyscrollbar( {
                    ...
                    slideCallback: function() { GalleryViewer.loadFilmStripThumbnails(GalleryViewer.getScrollOffset()) }
                });
            });
        }
        ...
        if (!(location.hash && location.hash != "")) {
            ...
            if (indices[0] == imageKey) {
                ...
                GalleryViewer.preloadImages([GalleryViewer.getNextIndex(0), GalleryViewer.getPreviousIndex(0)]);
                GalleryViewer.currentImage = document.getElementById("current-image");
                GalleryViewer.currentImage.style.display = "";
                }
