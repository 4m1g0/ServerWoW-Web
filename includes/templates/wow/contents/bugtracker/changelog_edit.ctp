	...
	<div style="padding:20px;width:720px;float:right;margin-top:-65px;">
		...
		?>
			<tr valign="top">
				...
				<td><?php if ($label[1] == 'text') : ?><input id="<?php echo $field; ?> type="text" class="input text" size=64 name="changelog[<?php echo $field; ?>]" value="<?php echo $item[$field]; ?>" />
				<?php elseif ($label[1] == 'textarea') : ?><textarea name="changelog[<?php echo $field; ?>]" id="<?php echo $field; ?>" class="input textarea" rows="10" cols="80"><?php echo $item[$field]; ?></textarea><?php endif; ?></td>
			</tr>
		<?php endforeach; ?>
			<tr>
