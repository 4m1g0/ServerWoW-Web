...
/**
    ...
    public function GetNpcAreaInfo($entry) {
    	$npc_coordinates = $this->c('Db')->model('Creature')->getItem($entry, array('guid', 'map', 'position_x', 'position_y'));
        if(!$npc_coordinates)
		{
            $this->c('Log')->writeDebug('%s : creature #%d was not found in `creature` table!', __METHOD__, $entry);
            return false;
        }
        $area_data = $this->c('QueryResult')
        	->model('Zones')
        	->addModel('Areas')
        	->fieldsConditions(array(
				array('wow_zones.map', ' = ' . $npc_coordinates['map']),
				array('wow_zones.y_min', ' <= ' . $npc_coordinates['position_y']),
				array('wow_zones.y_max', ' => ' . $npc_coordinates['position_y']),
				array('wow_zones.x_min', ' <= ' . $npc_coordinates['position_x']),
				array('wow_zones.y_max', ' => ' . $npc_coordinates['position_x']),
			))
			->loadItem();
        ...
        $area_data = $this->c('Db')
			->model('Zones')
			->findParams(array(
				'fields' => array('id', 'area', 'wow_areas.name')
			))
			->setJoin(array(
				array(
					'parent_table' => 'wow_zones',
					'parent_field' => 'area',
					'table' => 'wow_areas',
					'field' => 'id',
					'type' => '=',
			...
			->setConditions(array(
				array(
					'field' => 'map',
					'type' => '=',
					'value' => $npc_coordinates['map'],
					'typeAndOr' => 'AND'
				),
				array(
					'field' => 'y_min',
					'type' => '>=',
					'value' => $npc_coordinates['position_y'],
					'typeAndOr' => 'AND'
				),
				array(
					'field' => 'y_max',
				...
				array(
					'field' => 'x_min',
					'type' => '>=',
					'value' => $npc_coordinates['position_x'],
					'typeAndOr' => 'AND'
				),
				array(
					'field' => 'x_max',
	...
	{
		return $this->c('QueryResult')->model('Areas')->setItemId($zoneId)->fields(array('name'))->loadItem();
//		return $this->c('Db')->model('Areas')->getItem($zoneId, array('name'));
    }
}
?>