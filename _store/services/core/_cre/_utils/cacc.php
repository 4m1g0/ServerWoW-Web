...
function changeRealm()
{
	...
	document.reino.action = 'servicios.php?p=28&realm='+realm;
}
</script>
<div class="section-title">
    <span>Personalizacion de Personaje</span>
    <p>Hola <?php echo $_SESSION['username']; ?>, desde esta pagina puedes personalizar Tus personajes.</p>
</div>
<?
/*
...
if (isset($_POST['realm']) && isNum($_POST['realm']) && ($_POST['guid']==NULL))
{
	...
	if(dbrows($name) != 0)
	{
		...
		<tr>
		<td>
		<b>Customizacion de Personaje</b><br><br><br><br>
		<b>Personaje</b> - Selecciona el Personaje que Recibira la Recompensa.<br><br>
		...
		<td><input type="text" class="inp" name="bonusesfor" id="bonusesfor" value="'.$cre_reward[cacc][0].' Creditos" size="15" readonly style="width: 80%;"></td>
...
else if (isNum($_POST['realm']) && isNum($_POST['guid']) && isset($cre_reward['cacc']))
{
	...
	if($shard['cre'] >= $price)
	{
		...
		if(dbrows($char) != 0)
		{
			...
			switchConnection($_POST['realm'],"character");
			$char  = dbarray($char);
			dbquery("UPDATE characters SET `at_login`='8' WHERE guid='$_POST[guid]' limit 1");
			
			switchConnection(1,"realmd");
			$value="Customizacion de Personaje al personaje $char[name] :: Guid $_POST[guid]";
			history($value,$ac_id,'cacc',$price,$_POST['realm'],$_POST['guid']);
