...
function changeRealm()
{
	...
	document.reino.action = 'servicios.php?p=24&realm='+realm;
}
</script>
<div class="section-title">
    <span>Cambio de Nombre de Personaje</span>
    <p>Hola <?php echo $_SESSION['username']; ?>, desde esta pagina puedes modificar tu el Nombre de Tus personajes.</p>
</div>
<?
/*
...
if (isset($_POST['realm']) && isNum($_POST['realm']) && ($_POST['guid']==NULL))
{
	...
	if(dbrows($name) != 0)
	{
		...
		<tr>
		<td>
		<b>Cambio de Nombre</b><br><br><br><br>
		...
		<td><input type="text" class="inp" name="bonusesfor" id="bonusesfor" value="'.$cre_reward[rename][0].' Creditos" size="15" readonly style="width: 80%;"></td>
...
else if (isNum($_POST['realm']) && isNum($_POST['guid']) && isset($cre_reward['rename']))
{
	...
	if($shard['cre'] >= $price)
	{
		...
		if(dbrows($char) != 0)
		{
			...
			switchConnection($_POST['realm'],"character");
			$char  = dbarray($char);
			dbquery("UPDATE characters SET `at_login`='1' WHERE guid='$_POST[guid]' limit 1");
			
			switchConnection(1,"realmd");			
			$value="Cambio de Nombre al personaje $char[name]";
			history($value,$ac_id,'rename',$price,$_POST['realm'],$_POST['guid']);
