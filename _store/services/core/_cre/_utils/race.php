...
function changeRealm()
{
	...
	document.reino.action = 'servicios.php?p=25&realm='+realm;
}
</script>
<div class="section-title">
    <span>Cambio de Raza de Personaje</span>
    <p>Hola <?php echo $_SESSION['username']; ?>, desde esta pagina puedes modificar tu la Raza de Tus personajes.</p>
</div>
<?
/*
...
if (isset($_POST['realm']) && isNum($_POST['realm']) && ($_POST['guid']==NULL))
{
	...
	if(dbrows($name) != 0)
	{
		...
		<tr>
		<td>
		<b>Cambio de Raza</b><br><br><br><br>
		...
		<td><input type="text" class="inp" name="bonusesfor" id="bonusesfor" value="'.$cre_reward[race][0].' Creditos" size="15" readonly style="width: 80%;"></td>
...
else if (isNum($_POST['realm']) && isNum($_POST['guid']) && isset($cre_reward['race']))
{
	...
	if($shard['cre'] >= $price)
	{
		...
		if(dbrows($char) != 0)
		{
			...
			switchConnection($_POST['realm'],"character");
			$char  = dbarray($char);
			dbquery("UPDATE characters SET `at_login`='128' WHERE guid='$_POST[guid]' limit 1");
			
			switchConnection(1,"realmd");
			$value="Cambio de Raza al personaje $char[name] :: Guid $_POST[guid]";
			history($value,$ac_id,'race',$price,$_POST['realm'],$_POST['guid']);
