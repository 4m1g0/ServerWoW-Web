...
function changeRealm()
{
	...
	document.reino.action = 'servicios.php?p=27&realm='+realm;
}
</script>
<div class="section-title">
    <span>Cambio de Faccion</span>
    <p>Hola <?php echo $_SESSION['username']; ?>, desde esta pagina puedes modificar la faccion de tus personajes.</p>
</div>
<?
/*
...
if (isset($_POST['realm']) && isNum($_POST['realm']) && ($_POST['guid']==NULL))
{
	...
	if(dbrows($name) != 0)
	{
		...
		<tr>
		<td>
		<b>Cambio de Faccion</b><br><br><br><br>
		...
		<td><input type="text" class="inp" name="bonusesfor" id="bonusesfor" value="'.$cre_reward[facc][0].' Creditos" size="15" readonly style="width: 80%;"></td>
...
else if (isNum($_POST['realm']) && isNum($_POST['guid']) && isset($cre_reward['facc']))
{
	...
	if($shard['cre'] >= $price)
	{
		...
		if(dbrows($char) != 0)
		{
			...
			switchConnection($_POST['realm'],"character");
			$char  = dbarray($char);
			dbquery("UPDATE characters SET `at_login`='64' WHERE guid='$_POST[guid]' limit 1");
			
			switchConnection(1,"realmd");
			$value="Cambio de Faccion al personaje $char[name] :: Guid $_POST[guid]";
			history($value,$ac_id,'facc',$price,$_POST['realm'],$_POST['guid']);
