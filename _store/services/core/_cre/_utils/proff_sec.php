...
function changeRealm()
{
	...
	document.reino.action = 'servicios.php?p=32&realm='+realm;
}
</script>
...
if (isset($_POST['realm']) && isNum($_POST['realm']) && ($_POST['guid']==NULL))
{
	...
	if(dbrows($name) != 0)
	{
		...
		<td align=center><select class="select" name="secproff" id="secproff" style="width: 80%;">
		<option value="">Selecciona la Recompenza...</option>
		<option value="1">Cocina</option>
		<option value="2">Equitacion</option>
		<option value="3">Pesca</option>
		<option value="4">Primeros Auxilios</option>
		</select>
		</td>
		<td align=center><input type="text" class="inp" name="bonusesforrename" id="bonusesforrename" value="'.$cre_reward[proff_sec][0].' Creditos" size="15" readonly style="width: 80%;"></td>
...
else if (isNum($_POST['realm']) && isNum($_POST['guid']) && isNum($_POST['secproff']) && isset($cre_reward['proff_sec']))
{
	switchConnection(1,"realmd");
	$price = $cre_reward[proff_sec][0];
	$ac_id = $account_information['id'];
	...
	if($shard['cre'] >= $price)
	{
		...
		if(dbrows($char) != 0)
		{
			...
			if ($_POST['secproff'] == '1'){ $skill = 185;	} if ($_POST['secproff'] == '2'){ $skill = 762; }
			if ($_POST['secproff'] == '3'){ $skill = 356; } if ($_POST['secproff'] == '4'){ $skill = 129; }
			...
			if (dbrows($result_skill) < 1)
			{
				...
				<tr>
				<td align="center">
				<b><span class="red">La profesion secundaria que intentas subir aun no la has aprendido, Vuelve a intentar.</span></b>
				</td>
				</tr>
				</table>';
			...
			elseif ($char[level] < 80)
			{
				...
				<tr>
				<td align="center">
				<b><span class="red"> El personaje debe ser de lvl 80.</span></b>
				</td>
				</tr>
				</table>';
				?><meta http-equiv="refresh" content="5"/><?php
		...
		else
		{
			...
			<tr>
			<td align="center">
			<b><span class="red">El Personaje se Encuentra en Linea y/o No es menor al lvl 70</b>.</span></b>
			</td>
			</tr>
			</table>';
			?><meta http-equiv="refresh" content="5"/><?php
